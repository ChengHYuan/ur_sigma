...
#include "epos_driver/Definitions.h"
#include "epos_driver/EposMotor.hpp"
#include "epos_driver/utils/Macros.hpp"
#include "epos_driver/utils/EposException.hpp"
#include "epos_driver/control/EposProfilePositionMode.hpp"
#include "epos_driver/control/EposProfileVelocityMode.hpp"
#include "epos_driver/control/EposCurrentMode.hpp"
...
void EposMotor::init(ros::NodeHandle &root_nh, ros::NodeHandle &motor_nh, const std::string &motor_name)
{
    ...
    VCS_NODE_COMMAND_NO_ARGS(SetDisableState, m_epos_handle);//把设备设置为disable
    ...
    m_state_publisher = motor_nh.advertise<epos_msgs::MotorState>("get_state", 100);
    m_state_subscriber = motor_nh.subscribe("set_state", 100, &EposMotor::writeCallback, this);
}

epos_msgs::MotorState EposMotor::read()
{
    try {
        if (m_control_mode) {
            m_control_mode->read();
...
void EposMotor::writeCallback(const epos_msgs::MotorState::ConstPtr &msg)
{
...
{
    const DeviceInfo device_info(motor_nh.param<std::string>("device", "EPOS4"),//launch中的关键字，变量默认值
...
void EposMotor::initDeviceError()
{
    ...
    if (num_of_device_errors > 0) {
        throw EposException(m_motor_name + ": " + std::to_string(num_of_device_errors) + " faults uncleared");//当没出现" faults uncleared"时表示设备都正常运行
    }
}
...
void EposMotor::initPVC(ros::NodeHandle &motor_nh){
    const std::string control_mode(motor_nh.param<std::string>("control_mode", "profile_position"));
    if (control_mode == "profile_position") {
 ...
 */
double EposMotor::ReadVelocity()///读速度这里存在一点问题
{
    int raw_velocity;
    double velocity;
...
//        velocity = raw_velocity * M_PI / 30.;
        // rpm -> r/s
        velocity = (double)raw_velocity/(ratio* 60.);
    } else {

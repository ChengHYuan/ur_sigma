...
{
    ROS_INFO("Activate ProfilePosition Mode");
...
void EposProfilePositionMode::write(const double position, const double velocity, const double current)
{
    ...
    ROS_INFO("Target Position:  %f ",position);
...
//    clock_t startTime = ros::Time::now().toNSec();//纳秒
//    unsigned int error_code;

    VCS_NODE_COMMAND(MoveToPosition, m_epos_handle, quad_count, /* absolute */true, /* overwrite */true);
//    VCS_MoveToPosition(m_epos_handle.ptr.get(), m_epos_handle.node_id, quad_count, true, true, &error_code);
    // VCS_NODE_COMMAND(MoveToPosition, m_epos_handle, quad_count, /* absolute */false, /* overwrite */true);
//    clock_t endTime = ros::Time::now().toNSec();
//    double durationTime = (double)(endTime-startTime) / 1e6;
//    ROS_INFO("The write time is: %f  ms",durationTime);
}

...
vector<double>start_point = { 0.642,0.06,0.09 };
vector<double>target_point = { start_point[0] + 0.12,start_point[1]-0.12 ,start_point[2]};
...
Eigen::Quaterniond eula_to_quaternion(Eigen::Vector3d &eula) {
    ...
    return quaternion = yawAngle * pitchAngle * rollAngle;
}
...
void save_traj(dmp::DMPTraj &traj,int index) {//存储dmp点
    ofstream ofs; //定义流对象
    string file_name = "/home/chy/sigma_iiwa_simulation/sigma_to_sim_workspace/path/standard_path" + to_string(index) + ".txt";
...
int main(int argc, char **argv)
{
    ...
    if (clientLfd.call(srv)) {
        ...
        for (int i = 0;i < goal_indexes.size();++i) {
            ...
            Eigen::Vector3d goal_vector(goals[i][0] - start_point[0], goals[i][1] - start_point[1], 0);

            //计算旋转后的目标向量
            goal_vector =  goals_rotate[i]*goal_vector;
            //计算旋转后的目标位置，并且存储
            goal_box.push_back({ start_point[0] + goal_vector[0],start_point[1] + goal_vector[1], start_point[2] });
        ...
        if (clientActive.call(srvSetActive)) {
            ...
            for (int i = 3;i < 6;++i)srvPlan.request.goal.push_back(points[points.size()-1].positions[i]);//设置终止点位置
